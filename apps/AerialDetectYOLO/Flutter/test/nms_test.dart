...
import 'package:aerialdetect/services/nms.dart';
...
void main() {
  group('per-class NMS (IoU 0.45)', () {
    test('test_per_class_nms_keeps_overlapping_different_classes', () {
      // Two fully-overlapping boxes of DIFFERENT classes -> both survive.
      final List<Detection> dets = <Detection>[
        box(0, 0, 100, 100, 3, 0.9), // car
        box(0, 0, 100, 100, 5, 0.8), // truck
      ];
      final List<Detection> kept = nonMaxSuppressionPerClass(dets, 0.45);
      expect(kept.length, 2);
      expect(kept.map((d) => d.classId).toSet(), <int>{3, 5});
    });

    test('test_nms_suppresses_same_class_overlap', () {
      final List<Detection> dets = <Detection>[
        box(0, 0, 100, 100, 3, 0.9),
        box(5, 5, 105, 105, 3, 0.8), // IoU ~0.82 with the first
      ];
      final List<Detection> kept = nonMaxSuppressionPerClass(dets, 0.45);
