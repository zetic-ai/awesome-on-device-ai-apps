"""
AerialDetectYOLO — Stage 0 export recipe (re-runnable).

Model:   ENOT-AutoDL/yolov8s_visdrone  (Apache-2.0)
Weights: baseline_enot/weights/best.pt  (standard Ultralytics YOLOv8s, trained at imgsz 928
         on VisDrone-2019 — 10 aerial classes; the higher train resolution is what gives
         the small top-down-object recall a generic 640 COCO model lacks)
Family recipe: Ultralytics YOLO -> ONNX, STATIC shapes (dynamic=False), opset 12,
         half=False (Melange handles precision). Same recipe as PyroGuard, only imgsz differs.
...
Deps: pip install ultralytics onnx onnxslim huggingface_hub numpy dill
"""
import os
import numpy as np
import onnx
...
REPO_ID = "ENOT-AutoDL/yolov8s_visdrone"          # Apache-2.0
WEIGHT  = "baseline_enot/weights/best.pt"         # standard YOLOv8s, trained @ imgsz 928
IMGSZ   = 928                                     # divisible by 32 (928/32=29); STATIC, fixed
OPSET   = 12
ONNX_OUT = os.path.join(HERE, "aerialdetect-yolov8s-visdrone.onnx")

def main():
    # 1. Pull the chosen aerial-trained checkpoint from Hugging Face.
    ...
    exported = model.export(
        format="onnx",
        imgsz=IMGSZ,        # 928x928 static — suits small top-down aerial objects
        opset=OPSET,
        simplify=True,
        dynamic=False,      # STATIC SHAPES OR BUST
        half=False,         # Melange decides precision; keep ONNX fp32
    )
    # Move/rename the artifact next to this script.
    ...
    # Static-shape assertion: no symbolic (string) dims anywhere.
