...
/// One measured inference: the grading result plus how long `model.run` took.
...
/// Owns the Melange model lifecycle for the DR severity grader:
...
class MelangeService {
  ...
  /// separated by a slash: `<account>/<project>`. A bare name throws
  /// MlangeException(3) (confirmed on-device this run).
  static const String modelName = 'ajayshah/RetinaDRGrade';
  ...
  ///
  /// The served model is ~328 MB fp32 (ViT-base) — the first-run pull is a real,
  /// user-visible download. [onProgress] reports progress in [0, 1] for the
  /// loading screen.
  ...
  /// One dummy inference so the first real grading is not the slow (cold) one.
  ...
  ///
  /// Reads the raw `logits[1,5]` output and hands all five to the postprocessor,
  /// which applies softmax ONCE and takes the argmax as the grade.
  InferenceOutcome infer(Float32List inputData) {
    ...
    final logits = List<double>.generate(
      Postprocessor.numGrades,
      (i) => raw[i].toDouble(),
    );
    final result = postprocessor.classify(logits);
