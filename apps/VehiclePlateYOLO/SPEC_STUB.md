...
- Melange model name: `<<FILL AT GATE 0>>` (requested: ajayshah/VehiclePlateYOLO)
...
3. Confidence is already activated by the export head (a Sigmoid node is baked into
   the exported graph — confirmed by inspecting koushim-yolov8-license-plate.onnx) —
   apply NO extra sigmoid in Dart. Still assert against the real tensor in Tier A.
...
- Known OS traps: FP32-GPU CoreML artifact can crash in MPSGraph on iOS/macOS 26.3+;
  ...
  native console and confirm it is not GPU on affected OS versions. Realistic
  non-crashing fallback is TFLITE_FP16/CPU (hundreds of ms), not the NPU.
...
- Score semantics: confirm plate_conf needs NO extra sigmoid (Ultralytics export
  already activates) — test against the real tensor.
