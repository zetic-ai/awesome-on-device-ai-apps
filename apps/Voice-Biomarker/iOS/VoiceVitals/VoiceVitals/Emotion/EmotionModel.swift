...
/// (wav2vec2-base, Apache-2.0). Input: raw 16 kHz waveform `(1, 48000)`; output: 4 logits.
///
/// This is the cleanest stand-in for a mental-health vocal biomarker: the same
/// pipeline a client (Aiberry / Ellipsis) would use, just with their model.
final class EmotionModel: ObservableObject {
    ...
    private let queue = DispatchQueue(label: "voicevitals.emotion")

    // Label order matches the model's config.id2label:
    // [angry, disgust, fear, happy, neutral, sad, surprise].
    private let labels = ["Angry", "Disgust", "Fear", "Happy", "Neutral", "Sad", "Surprise"]
