...
object AudioUtils {
    ...
    /**
     * Fraction (0..1) of frames whose RMS is >= [relThreshold] x the loudest frame —
     * i.e. how much of the recording was actually voiced. Used by the fusion engine as a
     * rate-of-speech proxy. Mirrors `AudioUtils.voicedFraction` in iOS-Aiberry.
     */
    fun voicedFraction(
        x: FloatArray,
        frame: Int = 320,            // 20 ms @ 16 kHz
        ...
        while (i + frame <= x.size) {
            var sum = 0f
            for (j in i until i + frame) sum += x[j] * x[j]
            energies.add(sqrt(sum / frame))
            i += frame
        }
        if (energies.isEmpty()) return 0f
        val maxE = energies.max()
        if (maxE <= 1e-6f) return 0f
        val thr = maxE * relThreshold
        val voiced = energies.count { it >= thr }
        return voiced.toFloat() / energies.size
    }
}
